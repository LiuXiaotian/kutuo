...
namespace WarOfLords.Core.Models
{
    public abstract class BattleUnit : IFightCapability, IMovementCapability
    {
        ...
        public virtual MapVertex Position { get; set; }
        ...
        private async Task<int> MoveForAttack(MapVertex direction)
        {
            ...
            {
                this.Team.MessageQueue.EnqueueMessage(this, "Moving to {0} for attack", direction);
                int distance = direction.DistanceTo(this.Position);
                ...
                while (stepNeeded > 0 && !cancelToken.IsCancellationRequested &&  enemies.Count() == 0)
                {
                    ...
                    if (!cancelToken.IsCancellationRequested)
                    {
                        this.Position.Moved(direction, this.MoveStepLength, this.Team.BattleFieldMap);
                ...
                if(!cancelToken.IsCancellationRequested && stepNeeded <= 0)
                {
        ...
        public async virtual Task Attack(MapVertex targetPos)
        {
            ...
            while (!cancelToken.IsCancellationRequested  && this.Health > 0 && !this.Team.EnemyAllDead())
            {
                ...
                {
                    BattleUnit target = enemies.OrderBy(_ => _.Position.DistanceTo(this.Position)).FirstOrDefault();
